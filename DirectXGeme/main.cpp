...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
   ...
   //デバッグレイヤーをオンに
	...
	while (true) {
		...
		//commandList->SetPipelineState(obj1->pipelineState);
		//commandList->SetGraphicsRootSignature(obj1->rootSignature);
		////プリミティブ形状の設定コマンド
		//commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		////頂点バッファビューの設定コマンド
		//commandList->IASetVertexBuffers(0, 1, &obj1->vbView);
		...
		//commandList->IASetIndexBuffer(&obj1->ibView);
		////描画コマンド
		//commandList->DrawIndexedInstanced(_countof(obj1->indices), 1, 0, 0, 0);//すべての頂点を使って描画
